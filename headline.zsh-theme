...
# Behaviour when pressing <enter> without entering a command
# off     = print the normal prompt again
# on      = overwrite the previous prompt
# minimal = print only HL_EMPTY_PROMPT
HL_OVERWRITE='minimal' # on|off|minimal

# Prompt displayed when <enter> is pressed without a command
HL_EMPTY_PROMPT="  %{$reset%}$ "
...
headline-precmd() {
  ...
  # Detect <enter> without a command
  local empty_prompt='false'
  local overwrite='false'

  if [[ $_HL_CMD_NUM == $_HL_CMD_NUM_PREV ]]; then
    if [[ $HL_OVERWRITE == 'on' ]]; then
      overwrite='true'
    ...
    elif [[ $HL_OVERWRITE == 'minimal' ]]; then
      empty_prompt='true'
    fi
  fi
  ...
  if [[ $empty_prompt == 'true' ]]; then
    HL_OUTPUT_SEP=''
  elif [[ $HL_SEP_MODE == 'on' || ($HL_SEP_MODE == 'auto' && $_HL_AT_TOP != 'true') ]]; then
  ...
  if [[ $empty_prompt == 'true' ]]; then
    HL_OUTPUT_INFO=''
  elif [[ $HL_INFO_MODE == 'on' || ($HL_INFO_MODE == 'auto' && $information != $_HL_INFO) || $overwrite == 'true' ]]; then
